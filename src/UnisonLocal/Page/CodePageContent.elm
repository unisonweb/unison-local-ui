...
module UnisonLocal.Page.CodePageContent exposing (..)
...
fetchNamespaceDetails finishedMsg context perspective =
    ...
    in
    fqn_
        |> LocalApi.namespace context perspective
...
viewSidebar perspective cfg codebaseTree =
    let
        ...
        codeSection =
            Maybe.map
                (\c ->
                    Sidebar.section "Code" [ Html.map c.codebaseTreeMsg (CodebaseTree.view { withPerspective = True } c.codebaseTree) ]
